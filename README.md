...
**φ=0 的參考動作在 88% 的影格陷進地板 12–16 mm** —— 機器人一開始就被塞在地面裡。
...
| **上層 φ** | `RetargetNet`：把 β 映射成 36 個 retarget 參數 | 學 |
| **下層 ψ** | `LatentAdapter` + `ActionHead` + `RootWrenchHead` | 學 |
...
```
                             φ = RetargetNet(β)  ──► u ∈ R³⁶ ──► hard tanh box
                                                                      │
 (clip m, body β)                                                     ▼
      │                                                    apply_retarget
...
```
F(φ) = λ_gap  · G(τ, ref_φ)      機器人實際做出來的 vs 修正後參考   的差距
     + λ_fid  · S(ref_φ, src)    修正後參考       vs 原始成人動作   的偏離   ← 防退化主力
...
1. **硬 tanh box（結構防禦，不靠調權重）**
   ...
   而且 **φ = 0 精確重現原本的 naive retarget** —— 它是架構原點，不是靠權重維持的軟目標。
...
3. **診斷**：`u_saturation = mean(|tanh(u)| > 0.9)`，超過 0.2 就是退化警報。
   決定性檢驗：**`G(τ, ref_0)` 必須跟著 `G(τ, ref_φ)` 一起降** —— 只有後者降就是在作弊。

### 4.2 雙層形式與 hypergradient

```
下層:  ψ*(φ) ∈ argmax_ψ  J(ψ; φ) = E[ Σ_{t<24} γᵗ r(s_t, a_t; ref_φ) ]
上層:  min_φ  F(φ, ψ*(φ))
```

耦合是**雙通道**的：φ 同時進入 reward **和初始狀態分布**（RSI 從 `ref_φ[t0]` 起始）。

```
dF/dφ = ∂F/∂ref · ∂ref/∂φ              [T1]  autograd 精確，零變異 —— 訊號主體
      + ∂F/∂τ · ∂τ/∂ref · ∂ref/∂φ      [T2]  模擬器不可微 → 反對稱 ES 估計
      + ∂F/∂τ · ∂τ/∂ψ · dψ*/dφ         [T3]  捨棄
```

- **T1 保留**：這就是「retargeting 用 torch 重寫成可微分」的價值所在。
  但 `E_ext` 與 `P` 的 T1 **恆為零**（它們只透過模擬器依賴 φ）→ 純 T1 看不見外力作弊，這是 ES 要補的洞。
- **T3 捨棄**：需要在接觸不連續的 MDP 上做 Hessian-inverse-vector product，算不出來。
  **誠實的但書**：捨棄 T3 後解的**不是** bilevel 問題，而是 `min_φ F(φ, ψ_k)` 的 Gauss-Seidel 交替。
  代價是 φ 沒有動機去挑「現在難、但能養出更好策略」的參考 —— 對這個應用是特性不是缺陷。

**TTSA（兩個時標）**：`η_ψ = 3e-4`、`η_φ = 1e-5`（30:1），上層每 `K = 10` 輪才更新一次（→ 有效 300:1），
...
- **RSI 通道**：每個 window 的初始狀態**字面上就是** `ref_φ[t0]`。沒有 T2 的上層不知道
  「移動參考也會移動機器人每次的起點」。
- **外力作弊看不見**：`E_ext` 與 `P` 只透過模擬器依賴 φ，它們的 T1 恆為零。

作法 —— **擾動 φ 的「輸出」`u ∈ R³⁶`，不是它的 ~10k 個權重**（36 維才讓 ES 可行）：
...
σ_φ = 0.02（pre-tanh 空間，約 0.4% gain 變化、0.003 rad 偏移變化）
              ...
              1
  ĝ_u  =  ───────── Σ_g [ F̂_sim(u + σ_φ ε_g) − F̂_sim(u − σ_φ ε_g) ] · ε_g
           2 σ_φ G_b
...
> 同一對反對稱的兩次模擬，除了 `±σ_φ ε` 之外，`(哪段動作、window 起始幀、RSI 關節噪音、
> 整條 (24,75) 動作噪音)` 必須**逐位元相同**。ES 的訊號在 `G` 上只有約 10% 的相對變化，
> 沒有 CRN 就會被 rollout 噪音完全淹沒 —— **加了比不加還糟**。
> 驗證方法：令 `σ_φ = 0` 的一對必須產生逐位元相同的 `qpos` 軌跡。
...
**Stage 1 為什麼要解凍 `dz_root`**：φ 全凍時這階段贏不了。`φ=0` 的參考陷在地板裡 12–16 mm
...
且 `D` 對兩個參考各報一次（`D_φ` 與 `D_0`），這就是 §4.1 那個防作弊的決定性檢驗。
...
model/bilevel/
  ...
  upper.py          F(φ) 組裝、T1 精確路徑、T2 的 ES 路徑、信賴域
