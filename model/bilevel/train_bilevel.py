"""Bilevel training entry point: the TTSA loop.
    ...
    for k = 1 .. num_iters:
        collect 256 windows x 24 steps with (psi_k, phi_k)
        psi_{k+1} = PPO(psi_k)                                  every iteration
        accumulate the antithetic-ES estimate of dF_sim/du
        if k > upper_warmup_iters and k % upper_every == 0:
            phi_{k+1} = Adam(phi_k, T1 + ES/K + prox)            every K = 10

Timescale separation: eta_psi/eta_phi = 30, upper cadence K = 10, so the
effective ratio is 300:1. Borkar's TTSA conditions (sum eta = inf, sum eta^2 <
inf, eta_phi/eta_psi -> 0) are NOT literally satisfied by Adam -- the theory is
...
phi is frozen at 0 for the first upper_warmup_iters. F(phi, psi) is only
meaningful once psi is worth measuring, and freezing is the practical stand-in
for the "psi near psi*(phi)" that the analysis assumes.
...
def build(cfg, verbose=True):
    ...
    # ONE RetargetNet shared by every body. That sharing is what forces phi to
    ...
    return dict(
        ...
        opt=opt, assign=assign, test_tasks=test_tasks, long_eval=long_eval,
    )
...
class WandbLogger:
    ...
    def log_calibration(self, scales, baseline):
        """The phi=0 / box-corner table from UpperLevel.calibrate(), once.
        ...
        if not self.enabled or not scales:
            return
        try:
            tbl = self.wandb.Table(columns=["term", "phi0", "box_corner", "normalized_at_phi0"])
...
def _wandb_metrics(m: dict, stage: int) -> dict:
    ...
    upper = {"F", "G", "S", "C", "prox", "frac_illegal", "u_saturation", "u_absmax",
             "dz_root", "ref_min_z",
             "u_step_linf", "u_step_clipped", "es_grad_norm", "es_delta"}
    sched = {"lambda_bc", "wrench_scale"}
    long = set(LONG_KEYS)
    out = {"stage": stage}
    for k, v in m.items():
...
def init_from_checkpoint(path, C, verbose=True):
    ...
    Stage 1 -> Stage 2 is a different thing: the objective itself changes (phi
...
def apply_stage(cfg, stage: int) -> BilevelConfig:
    ...
    if stage == 1:
        ...
        #
        # ONE exception to "phi frozen": dz_root is free. Freezing all of phi
        # was measured to make this stage unwinnable. The phi=0 reference sits
        ...
        # a learned dz_root means freezing phi also freezes the only thing that
        ...
        # Adds morphology diversity AND the remaining 35 phi dimensions. The
        ...
        # exact T1 gradient is blind to wrench-cheating (E_ext depends on phi
...
def train(cfg, stage: int, verbose=True, resume=None, init_from=None):
    ...
    # Measure every S/C term at phi=0 and adopt it as that term's unit. Without
        ...
        for it in range(start_iter, cfg.num_iters):
            ...
            if it < cfg.value_warmup_iters:
                ...
                m = update_lower(cfg, it, policy, value_net, opt, ep,
                                 pair_norm, ret_norm, bc_targets=bc)
