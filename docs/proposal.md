...
> 上層真正要修的不是關節限位，而是**參考動作陷進地板裡**（φ=0 時 88% 的影格
...
| 符號 | 唸法／全名 | 是什麼 |
|---|---|---|
| **φ** | phi | **上層**的參數。一個小網路的權重，決定參考動作怎麼被修正 |
| **ψ** | psi | **下層**的參數。三個小網路（見下）的權重，決定機器人怎麼動 |
...
| **u** | — | φ 網路吐出的 36 個原始數字，經過硬 tanh box 後變成實際的 retarget 參數 |
...
| 名詞 | 白話解釋 |
|---|---|
| **policy（策略）** | 「看到什麼狀態就做什麼動作」的函數。這裡就是 ψ 那三個網路加上凍結 AI |
...
| **advantage `A`** | 「這個動作比平均好多少」＝ 實際拿到的 − value 預測的。**PPO 用它決定要加強還是抑制某個動作** |
...
| **Gauss-Seidel 交替** | 捨棄 T3 之後實際在做的事：固定 ψ 優化 φ、固定 φ 優化 ψ，來回輪流。**它的解跟真正的 bilevel 解不同**，§4.3 有誠實說明代價 |
...
| **hard tanh box（硬 box）** | 把 φ 的輸出過 `tanh` 再乘一個固定寬度，讓每個 retarget 參數**在結構上不可能**超出範圍。防退化的第一道防線，不靠調權重 |
| **saturation（飽和）** | `|tanh(u)| > 0.9` 的比例。表示 φ 貼在 box 的牆上、想出去但出不去 —— **超過 0.2 就是退化警報** |
| **box-corner normalization** | 每個 S/C 分項的單位，取「φ 走到 box 角落時該項的值」。沒有這一步，各項的原始尺度差到 10¹¹ 倍，權重完全沒有意義（§11 偏差 B） |
...
| `u_saturation` | **退化警報**。φ 的輸出貼在硬 box 牆上的比例 | **> 0.2 就是危險訊號** |
| `u_absmax` | φ 輸出的最大絕對值 | 觀察用 |
...
> 前者 > 0.2 表示目標函數在把 φ 往退化推（§3.5）；
> 後者是「上層到底有沒有解決它該解決的問題」的唯一直接答案。

#### `upper_terms/` — 上層把預算花在哪一項

S、C、G 的逐項拆解。全部已按「φ 在硬 box 邊界能達到的值」正規化，所以都落在 [0,1]，
...
> 實測 φ=0 時 `c_smooth` 已經在 0.94、`s_froude` 在 0.47 —— 代表 φ 對這兩項幾乎沒有施力空間。
> 反過來 `c_limit` 在 0.006、`s_contact` 在 0.069，那才是 φ 真正能動的地方。
...
| 參考動作 φ | **永久凍結在 0**（`upper_warmup_iters = 10⁹`），就是 naive retarget |
| 外力 | **常開不退火**（`wrench_hold_iters = 10⁹`）|
| ES | 關 |

**為什麼**：這一階段在回答一個最基本的問題 —— **下層到底追不追得動？**
所以參考動作必須是靜止不動的靶（φ 凍結），外力這根拐杖必須一直在（不退火），
...
| φ **解凍** | 前 500 iter 仍凍結，之後每 10 iter 更新一次 |
| 外力 | **仍然常開** |
| ES | **仍然關** |
...
**為什麼 φ 還要再等 500 iter**：`F(φ, ψ)` 的意思是「在**這個** ψ 之下，參考動作該怎麼修」。
如果 ψ 還是隨機的，量到的差距反映的是「策略還沒學好」而不是「參考動作有問題」，
上層會朝著錯誤方向修。等 ψ 在新身材上先站穩，`F` 才有意義 —— 這也是
TTSA 理論裡「ψ 接近 ψ*(φ)」那個假設的實務替代品（§4.2）。

**門檻**：`ref_min_z ≥ −0.005 m`（參考不再陷進地板）、`u_saturation < 0.2`、`S < 2`，
**而且 §3.5 的誠實性檢驗要過** —— `D` 對 φ=0 參考也要降，不能只有對 φ 參考降。
...
> （φ=0 時 88% 影格陷入地板 12–16 mm），所以門檻換成直接量那件事。
...
`E_ext` 與 `P` 只透過模擬器依賴 φ，而模擬器不可微分，所以 T1 對它們恆等於零。
...
早期草稿有一個「Stage 4 —— 全量」，已經移除。原因很簡單：**規模是 `--iters`，不是階段**。
Stage 2 和 3 本來就已經用滿全部身材與全部 clip，唯一還能放大的只有迭代數，
所以那個 preset 與 Stage 3 逐欄位比對**零差異**，只多出一個 checkpoint 前綴。

**全量跑 = Stage 3 給滿 `--iters 10000`。**
...
> **看不懂的名詞一律回 §0.6 查。** 那裡有六張表：符號（φ/ψ/β/G/S/C…）、下層三個網路、
...
**這是 upper level 最強的存在理由。** 同時它讓「防退化」問題比表面上容易得多：φ 有一大塊**真實的、可量測的**修正空間可以先吃掉，還遠遠碰不到動作語意。
...
(clip m, body β)
  │
  ├─ src_qpos (T,76) ──► RetargetNet φ=f(β) ──► apply_retarget ─┬─► ref_raw   (可微，餵 feasibility penalty C)
  ...
  └─ z0 (256,) ──► LatentAdapter ──► project_z ──► z_beta ──► frozen FB actor ──┬─► ActionHead ──► μ_ctrl (69)
                        ↑ β                                                     └─► RootWrenchHead ──► μ_wrench (6)
                          ...
                          ┌───────────────────────────────────┴──────────────────────────────┐
                          ▼                                                                   ▼
              Lower: PPO + GAE (更新 ψ 每 iter)                        Upper: F(φ) (更新 φ 每 K=10 iter)
              ψ = {LatentAdapter, ActionHead, RootWrenchHead}          φ = RetargetNet 權重
```
...
`new.md` 說「Upper 就是要衡量機器人實際做出來的狀態跟參數化的參考動作 gt 之間的差距」。直接寫成 `min_φ E‖s_robot − ref_φ‖²` 會退化，原因很具體：

> 能到達 φ 的可微梯度**唯一**提供的方向，就是 `∂/∂ref ‖τ − ref‖² · ∂ref/∂φ`，字面上是「把 ref 搬到 τ 身上」。

這不是邊角案例 — 這是可解析部分的**全部內容**。本節其餘所有東西都在對抗這一項。

但退化受**容量**限制，而這件事很重要：

> **φ = f(β) 是每個身材 36 個全域數字，橫跨 540 個 clip、所有 window 位置、所有 timestep 共用。** 整個 upper level 的有效自由度約 360，對上 5400 個 (motion, body) pair。

36 個全域數字沒辦法把 540 段風格迥異的動作變成站姿。真正會發生的失敗不是塌成平凡解，而是**系統性振幅偏移**：φ 把所有 joint gain 往 box 下緣壓（`g → 0.8`）、把 `dz_root` 壓低，因為低振幅、貼地、被阻尼的動作一律比較好追。以下設計要防的是這個。
...
1. **φ = 0 精確重現現有的 `retarget_qpos()`**。`h_tgt/h_src` 直接沿用 `scripts/qpos_retarget.py:83 load_root_rest_height`。naive retarget 是**架構原點**，不是一個靠權重維持的軟目標。這是不會失效的那種保證。
2. **在 box 角落**：root scale 偏差 ≤16%、每個關節振幅 ≤20% 且偏移 ≤0.15 rad。塌成站姿在**結構上不可能**，不是「被懲罰」而是「做不到」。
3. **β-only 條件**：`RetargetNet(β) → u`，沒有 clip identity、沒有 timestep、沒有 state。逐 clip 或逐幀的 φ 會立刻退化。
...
```
F(φ) = λ_gap  · G(τ, ref_φ)          # new.md 要的：robot 與參考的差距
     + λ_fid  · S(ref_φ, src)        # 語意保真（對 SOURCE clip，身材不變量）
...
**接觸樣態項是主力**：如果 φ 壓低振幅或降低 root，腳離地/著地的時序會立刻位移，這一項馬上點火。這正是退化最先破壞的東西。

`S` 只依賴 φ，梯度**精確且零變異**。

#### C — 參考在目標身體上的可行性（純 φ，梯度精確）
...
這兩項**必須放在 F 裡，不能只放在 lower reward**。否則會形成迴圈：φ 把參考變難 → lower level 用 100 N 的外力硬撐 → 差距照樣小 → φ 完全不用付代價。

兩項都只透過 τ 依賴 φ，所以對**精確梯度貢獻為零** — 它們由 §4.4 的 ES 估計器涵蓋。Stage 1–2 若不開 ES，就只能靠硬 box 兜底。
...
| `S(ref_φ) / S(ref_0)` | < 2.0 | 語意漂移量 |
| `ref_min_z`（參考最低幾何點的高度） | ≥ **−0.005 m** | **upper level 的頭條成功指標**（2026-08-10 改；理由見下） |
| `frac_illegal_frames(ref_φ)` | 下降即可，**不設門檻** | 見 §0.3 的修正註記：0.958 是「至少一個關節越界」，但平均只超 0.6°，壓到 0.2 既不必要也不代表品質 |
| `mean(g_joint)` per group | 不應單調趨向 0.8 | 14 個 group 一起往下漂 = 振幅萎縮失敗模式 |
| `G(τ, ref_0)` vs `G(τ, ref_φ)` | **兩者都要降** | **決定性檢驗** |

最後一項最重要：**如果 `G(τ, ref_φ)` 降而 `G(τ, ref_0)` 不降，upper level 就是在作弊** — 差距是靠移動參考關掉的，不是靠機器人變好。多算一次 torch FK 就能得到，成本可忽略。
...
```
Lower:  ψ*(φ) ∈ argmax_ψ  J(ψ; φ) = E_{(m,β)~D, w~Window, τ~p_ψ(·|ref_φ)} [ Σ_{t=0}^{23} γᵗ r(s_t, a_t; ref_φ) ]
Upper:  min_φ  F(φ, ψ*(φ))                                                    # §3.4
```

- `ψ = {LatentAdapter, ActionHead, RootWrenchHead}`（value net `ξ` 是輔助的，不屬於 bilevel 變數）
- `φ = RetargetNet 權重`
- `H = 24`

**耦合是雙通道的**，這點不尋常且重要：φ 同時進入 lower level 的 **reward** `r(·; ref_φ)` **和初始狀態分布**（RSI 從 `ref_φ[window_start]` 起始）。後者是一個 episode 內的直接依賴 — 見 §4.3 的 T2。

### 4.2 TTSA 更新規則

```
for k = 1 .. 10000:
    以 (ψ_k, φ_k) 收集 256 windows × 24 steps                  # §6
    ψ_{k+1} = Adam_ψ(ψ_k, ĝ_ψ),  η_ψ = 3e-4 → 1e-4 (cosine)     # PPO, §5
    把 ĝ_u^{ES} 累積進 upper-gradient buffer
    if k > 500 and k % K == 0:
        φ_{k+1} = Adam_φ(φ_k, T1 + ĝ_u^{ES}/K + prox),  η_φ = 1e-5
        硬夾 ‖u_φ − u_φ^{prev}‖_∞ ≤ 0.02
        u_φ^{prev} ← u_φ
```

| 旋鈕 | 值 | 理由 |
|---|---|---|
| `η_ψ / η_φ` | 3e-4 / 1e-5 = **30 : 1** | 時標分離 |
| Upper cadence `K` | **10** | 有效時標比 300 : 1 |
| φ warmup | **前 500 iter 凍結在 φ=0** | ψ 得先值得被量測，`F(φ,ψ)` 才有意義。這是「ψ 接近 ψ*(φ)」的實務替代品。 |
| `λ_prox` | 1.0，外加每步 `‖Δu‖_∞ ≤ 0.02` 硬夾 | 界定每個 upper step 造成的 reward 非平穩程度 |

**誠實註記（要寫進 `train_bilevel.py` 的 module docstring）**：Borkar 的 TTSA 條件（`Ση=∞`、`Ση²<∞`、`η_φ/η_ψ→0`）在 Adam 下並不嚴格成立。理論只是比例的指引；真正提供穩定性的是 proximal 項 + 硬夾 + cadence。不要在程式碼註解裡宣稱收斂保證。

### 4.3 Hypergradient 分解 — 保留什麼、捨棄什麼

令 `r = ref_φ`，`τ` 為實際 rollout：

```
dF/dφ  =  ∂F/∂r · ∂r/∂φ                        [T1]  直接項，參考側
       +  ∂F/∂τ · ∂τ/∂r · ∂r/∂φ                [T2]  模擬器對參考的 episode 內反應
       +  ∂F/∂τ · ∂τ/∂ψ · dψ*/dφ               [T3]  真正的 hypergradient（學習反應）
```

**T1：完整保留。** `∂r/∂φ` 是 `RetargetNet` + `apply_retarget` 的 autograd。`∂F/∂r` 對 `S`、`C`、`λ_prox` 是精確的（它們不依賴其他東西），對 `G` 是給定 τ 下精確的（把實際 τ 當常數張量）。**零變異**。這是訊號主體，也正是「retargeting 用 torch 實作」的價值所在。

> ⚠️ 關鍵：`λ_ext·E_ext` 與 `λ_phys·P` 的 **T1 恆等於零** — 它們只透過模擬器依賴 φ。所以純 T1 路徑**看不見外力作弊**。這正是 ES 要補的洞。

**T2：用反對稱 ES 估計。** T2 不小：`∂τ/∂r` 包含 RSI 通道，那裡初始狀態**字面上就是** `ref_φ[t0]`。忽略 T2 的 upper level 不知道「移動參考也會移動機器人每個 window 的起點」。而 `∂τ/∂r` 恰恰是 MuJoCo 給不出來的東西。見 §4.4。

**T3：捨棄。** 理由依重要性排序：

1. `dψ*/dφ = −(∇²_ψψ J)⁻¹ ∇²_ψφ J` 需要在一個接觸不連續的 MDP 上做 RL 目標的 Hessian-inverse-vector product。連 `∇_ψ J` 本身都只有 score-function 估計；二階估計的變異隨 `1/σ⁴` 爆炸，在每 iteration 6144 個樣本下不可用。沒有 MJX/jax 就沒有可微模擬器的替代路線。
2. 這是標準的一階／截斷 hypergradient 近似（first-order MAML、first-order DARTS、one-step-unrolled bilevel 都是同一招）。已知在「lower level 對 upper 變數的反應夠平滑」時可行 — 這裡由建構保證，因為 φ 的硬 box 界定了 reward 目標能移動多遠。
3. 在時標分離下，該修正項通常是幅度／曲率修正，不是下降方向的符號翻轉。

**誠實的但書（同樣要寫進 docstring）**：捨棄 T3 之後我們解的**不是** bilevel 問題，而是 `min_φ F(φ, ψ_k)` 搭配 `ψ_k ← RL(φ_k)` 的 Gauss-Seidel 交替。其駐點與 bilevel 的駐點不同。具體代價：**φ 沒有動機去挑一個「現在難、但能養出更好 policy」的參考動作**。對這個應用而言這是特性不是缺陷 — 我們要的是忠實且可行的參考，不是課程規劃；而忠實與可行正是 T1 的 `S` 與 `C` 所編碼的。

### 4.4 T2 的 ES 估計器

擾動 φ 的**輸出** `u ∈ R^36`，不是它的 ~10k 權重。36 維讓 ES 變得可行。
...
- `σ_φ = 0.02`（pre-tanh 空間，約 0.4% gain 變化、0.003 rad 偏移變化）
- 只作用在依賴模擬器的部分 `F_sim = λ_gap·G + λ_ext·E_ext + λ_phys·P`：

  ```
  ĝ_u = (1/(2 σ_φ G_b)) Σ_g [ F̂_sim(u + σ_φ ε_g) − F̂_sim(u − σ_φ ε_g) ] · ε_g
  ```
...
> 同一個 antithetic pair 內，`(clip, window_start, RSI 關節噪音, 整條 (24,75) action noise stream)` 必須**逐位元相同**，只有 `±σ_φ ε` 不同。每 iteration 預先取樣 `(256,24,75)` 的 action noise，配對的列共用同一份。
> 沒有 CRN 的話，ES 訊號（在 `G` 上約 10% 相對變化）會被 rollout 噪音完全淹沒，該項會比不加還糟。
> **驗證方法**：令 `σ_φ = 0` 的 pair 必須產生逐位元相同的 `qpos` 軌跡。
...
| `reset_mask` | (256,) | u8 | |
| `ref_qpos` | (256, 25, 76) | f64 | φ 調整後、已 clamp 的參考 window；只在 window reset 時寫。3.9 MB |
...
├── retarget.py         256   φ = RetargetNet + 硬 tanh box
├── semantics.py        268   S（語意保真）與 C（可行性）
├── data.py             306   clips × bodies × windows 取樣
├── policy.py           165   ψ = 三個 head 掛在 frozen actor 上
...
| `retarget.py` | `RetargetNet(β(8)) → u(36)`；`RetargetParams(u)` → 硬 box；`apply_retarget(src, params)` → `(ref_raw, ref)` | `ref_raw` 未 clamp、保留範圍外梯度給 `C`；`ref` 已 clamp、給 RSI 與 tracking reward。**φ=0 逐位元重現 `qpos_retarget.py:91`**。69 個 hinge 綁成 14 組、左右共用 |
...
| `upper.py` | `UpperLevel.calibrate()` `.evaluate()` `.step()` `.accumulate_es()`；`EsPlan` | `calibrate` 以「φ 在 box 邊界的值」為每項單位；`evaluate` 組 F(φ) 並掛上 T1 精確圖；`step` 反傳 + L∞ trust region 回溯投影；`EsPlan` 管 CRN 配對與 per-body 擾動組 |
...
| 檔案 | 說明 |
|---|---|
| `rollout.py` | `Collector.collect()` 驅動 SimPool 走 H=24 步，主進程負責：由 φ 建參考、RSI 噪音取樣、CRN action noise、root-local wrench 旋到 world、終止後 mask。模擬用的參考是 detached；`upper.py` 需要梯度時自己重算 |
| `train_bilevel.py` | `build()` 接線、`apply_stage()` 套 Stage 1–4 預設、`train()` 跑 TTSA 主迴圈 |
| `tests/` | `test_torch_kin.py`（13 具 × 隨機姿態 vs `mj_kinematics`）、`test_retarget.py`（φ=0 錨點 + box + 梯度）、`test_simpool.py`（RSI/外力/終止/吞吐）。三者都是**擋路的門檻**，不是回歸測試 |
...
| 舊 | 新 |
|---|---|
| `scripts/qpos_retarget.py:91 retarget_qpos` | `bilevel/retarget.py` 的 φ=0 特例 |
...
**警報指標**：`frac_saturated > 0.2`。**決定性檢驗**：`G(τ, ref_0)` 必須跟著 `G(τ, ref_φ)` 一起降。
...
**R4 — 非平穩 reward 破壞 PPO。** 每次 φ 更新都會位移 value target。緩解：`K=10` + `η_φ=1e-5` + proximal + L∞ 硬夾。**監測**：每次 upper step 之後的 value loss；若尖峰 >2× 就加大 K。
...
**R7 — 對目標身體根本不可能的參考幀。** headstand 重定向到 `short_stocky` 可能無論 φ 怎麼調都起不來。24 步 window 下 policy 永遠學不到從那裡恢復，它會變成永久噪音源。緩解：per-(clip, body) advantage 正規化（§5.3），並可選擇在前 1000 iterations 課程式過濾掉最難的 10% pair。`model/diagnose_single_task.py` 已經懷疑過舊系統有這個混淆，別讓它靜默重演。

**R8 — RSI 噪音造成穿透的初始狀態**，在寬體身材上最嚴重。緩解：§6.4 的 clip + `ncon`/`contact.dist` 檢查 + 重試。

**R9 — ES 訊號被 rollout 噪音淹沒。** 有限差分在 `G` 上約 10% 相對變化。**CRN 不是可選項。** 若 ES 指標看起來像白噪音，用「`σ_φ=0` 的 pair 必須產生逐位元相同的 qpos」來驗證 CRN。
...
**Stage 1 — 只跑 lower level，φ 凍結在 0（承重階段）**
...
開啟 φ：`λ_gap + λ_fid + λ_feas + λ_prox` 加硬 box，`K=10`，`η_φ=1e-5`，500 iteration warmup。ES 關閉。

**通過門檻**：
- `ref_min_z ≥ −0.005 m`（參考不再穿地；取代原本的 `frac_illegal < 0.2`，見 §0.9 Stage 2）
- `S(ref_φ) < 2·S(ref_0)`
- `frac_saturated < 0.2`
- **且 `G(τ, ref_0)` 有降，不只是 `G(τ, ref_φ)`**
...
| **φ=0 錨點**：`apply_retarget(u=0)` vs `qpos_retarget.py:91` | **PASS @ 2.2e-16**（float64 eps） |
...
**有效解法：`scripts/calibrate_actuators.py`** —— 不用預測，直接**量測**。對真實 clip 的 φ=0
...
本文件 §3.4 給的權重（`c_limit=4.0` 等）在原始尺度下**完全不成立**。實測 φ=0 時：
...
第一版修法（按 φ=0 值正規化）也錯：`s_heading` 在 φ=0 是 **2.3e-32**（root 四元數原封複製，
轉向速率恆等）、`c_float` 是 **0**，除下去權重變 1e11。

**正解**：以 **φ 在硬 box 邊界能達到的值**作為每項的單位（`UpperLevel.calibrate()`）。
每一項因此在 φ 的整個可達集合上落在 [0,1]，config 的權重才真的是相對優先序。
副產品是一張有用的診斷表 —— φ 對各項的槓桿：

```
c_limit    φ=0 0.006 → corner 1.0   (30× 空間，符合設計意圖)
s_contact  φ=0 0.069 → corner 1.0   (22× 槓桿，主力項有效)
c_slide    φ=0 0.780               (φ 幾乎無法改善腳滑)
c_smooth   φ=0 0.940               (φ 對平滑度基本無影響力)
s_froude   φ=0 0.434
```

同時 §8.2 Stage 2 的門檻「`S(ref_φ) < 2·S(ref_0)`」現在可直接讀成 `S < 2 × 0.56`。
...
- `D_phi` —— 對 upper level 產生的 φ 參考
- `D_0` —— 對 naive φ=0 參考

這就是 §3.5 的決定性反退化檢驗。`D_phi` 降而 `D_0` 不降 = upper level 靠移動參考關掉差距，
不是機器人變好。腳本會直接印警告。
...
`scripts/qpos_retarget.py` 刻意保留 —— 它是 φ=0 的權威定義，
